...
unsigned int stringHash(void *s) {
  ...
  unsigned int ret = 0;
  for (int i = 0; i < len; i ++) {
    ret += (unsigned int)string[i];
  }
  return ret % dictionary->size;
}
...
void readDictionary(char *dictName) {
  ...
  char* new_word = malloc(sizeof(char*)*100);
  while(fscanf(fp, "%s", new_word) != EOF) {
    char* add_word = malloc(sizeof(char*)*strlen(new_word));
    strcpy(add_word, new_word);
    if (!findData(dictionary, add_word)) {
      insertData(dictionary, add_word, add_word);
    }
  }
  free(new_word);
  fclose(fp);
...
void processInput() {
  // -- TODO --
  char* str1 = malloc(61*sizeof(char));
  char* str2 = malloc(61*sizeof(char));
  char* str3 = malloc(61*sizeof(char));
  char c = 0;
  int i = 0;
